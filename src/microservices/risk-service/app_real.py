...
def load_model():
    ...
    # Create fallback model first to ensure service starts
    model = RiskAssessmentModel()
    logger.info("🔧 Initialized fallback model")
    
    try:
        # Try to load from MLflow with timeout
        ...
        try:
            ...
            if os.path.exists(model_path):
                # model.load_model(model_path)  # Skip this for now
                logger.info("✅ Local model file found (using fallback implementation)")
                return True
            else:
